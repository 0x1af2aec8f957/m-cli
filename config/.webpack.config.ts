...
const packageInfo = require(path.resolve(workDir, 'package.json'));
...
if (fs.existsSync(path.join(workDir, '.env'))) Object.assign(envVariable, Dotenv.parse(<string | Buffer>fs.readFileSync(path.join(workDir, '.env')))); // 加载自定义环境变量配置
if (isDevelopment && fs.existsSync(path.join(workDir, '.env.development'))) Object.assign(envVariable, Dotenv.parse(<string | Buffer>fs.readFileSync(path.join(workDir, '.env.development')))); // 开发环境加载环境变量配置
if (isProduction && fs.existsSync(path.join(workDir, '.env.production'))) Object.assign(envVariable, Dotenv.parse(<string | Buffer>fs.readFileSync(path.join(workDir, '.env.production')))); // 生产环境加载环境变量配置
...
/* const styleLoaderOptions: object = { // vue-style-loader
            ...
            modules: true,
            // 自定义生成的类名
            localIdentName: '[local]_[fullhash:base64:8]'
        };
...
config.module // ts-loader
    ...
    .loader('ts-loader')
    .options({
        context: workDir, // 设置ts配置文件的相对根目录
        transpileOnly: true, // 为true会解决ts文件热更新异常的问题,但会丢失类型检查
        appendTsSuffixTo: ['\\.vue$'],
        compilerOptions: { // 强制覆盖tsconfig配置项
            sourceMap: isDevelopment
        },
        configFile: ((): string => { // 同时适用cli和项目
            const CONFIG_NAME: string = '.tsconfig.json';
            const cliConfigPath: string = path.resolve(__dirname, CONFIG_NAME);
            const workPath: string = path.resolve(workDir, CONFIG_NAME);
            return fs.existsSync(cliConfigPath) ? cliConfigPath : workPath;
        })(), // 设置ts配置文件的位置
...
config // 插件项
    ...
    .end()
    .plugin('eslint')
    .use(ESLintPlugin, [{ /// doc: https://eslint.org/docs/developer-guide/nodejs-api#%E2%97%86-new-eslint-options
        extensions: ['js', 'vue', 'ts', 'jsx'], // 检测的文件扩展名称
        fix: true, // 自动修复错误
        useEslintrc: false,
        baseConfig: require(((): string => { // 同时适用cli和项目
            const CONFIG_NAME: string = '.eslintrc.json';
            const cliConfigPath: string = path.resolve(__dirname, CONFIG_NAME);
            const workPath: string = path.resolve(workDir, CONFIG_NAME);
            return fs.existsSync(cliConfigPath) ? cliConfigPath : workPath;
        })()),
    ...
    .end()
    .plugin('stylelint')
    .use(StyleLintPlugin, [{ /// doc: https://stylelint.io/user-guide/usage/options
        files: '**/*.(vue|htm|html|css|style|styl|less|(s(c|a)ss))', // 检测的文件扩展名称
        fix: true, // 自动修复错误
        config: require(((): string => { // 同时适用cli和项目
            const CONFIG_NAME: string = '.stylelintrc.json';
            const cliConfigPath: string = path.resolve(__dirname, CONFIG_NAME);
            const workPath: string = path.resolve(workDir, CONFIG_NAME);
            return fs.existsSync(cliConfigPath) ? cliConfigPath : workPath;
        })()),
    ...
    .use(ForkTsCheckerWebpackPlugin, [{ // 单开线程进行类型检查
        ...
        eslint: {
            enabled: false, // 禁用eslint规则检测，使用ESLintPlugin进行检测
            files: '**/*.{ts,tsx,js,jsx,vue}', // ForkTsCheckerWebpackPlugin需要的检测文件名称
            options: config.plugin('eslint').get('args')[0],
        }
    }])
    .end()
    .end();
...
config.devServer // doc: https://github.com/chimurai/http-proxy-middleware#proxycontext-config
    .hot(true)
    // .inline(false) // 为true将首次打开的展示是项目，不是/webpack-dev-serve
    .host('0.0.0.0')
    // .port(9000)
...
// [loader]配置依赖项从何处加载、解析
config.when(
    <boolean>fs.existsSync(path.resolve(__dirname, '..', 'node_modules')), // 确保配置弹出后依然可用
    (config: Config): void => {
        config.resolveLoader.modules // webpack处理loader代码查找的目录
            .add(path.resolve(__dirname, '..', 'node_modules')) // cli运行时
            .add(path.resolve(workDir, 'node_modules')) // webpack项目运行时
            .prepend(path.resolve(__dirname, '..', 'node_modules')) // cli运行时前置
            .end();
    }
);
...
// [typescript]支持外部的webpack-chain配置修改
config.when(<boolean>fs.existsSync(path.resolve(workDir, 'webpack.custom.ts')), (config: Config) : void => {
    require(path.resolve(workDir, 'webpack.custom.ts')).default(config); // 外部修改config基础配置，待优化
});

// [javascript]支持外部的webpack-chain配置修改
config.when(<boolean>fs.existsSync(path.resolve(workDir, 'webpack.custom.js')), (config: Config) : void => {
    require(path.resolve(workDir, 'webpack.custom.js'))(config); // 外部修改config基础配置，待优化
});
